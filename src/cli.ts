...
program
  ...
  .action(async () => {
    ...
    console.log(`\n  Run \x1b[33mmcpgen add <name>\x1b[0m to configure one.\n`);
  });
...
program
  ...
  .action(async (name: string, options: { config: string }) => {
    ...
    if (!entry) {
      console.error(`\x1b[31m✗\x1b[0m API "${name}" not found in registry. Run \x1b[33mmcpgen list\x1b[0m to see available APIs.`);
      process.exit(1);
    }
    ...
    mcpServers[`mcpgen-${entry.name}`] = {
      command: "npx",
      args: ["mcpgen", "serve", entry.specUrl, "--base-url", entry.baseUrl],
